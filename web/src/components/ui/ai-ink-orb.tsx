...
export function AiInkOrb({ label = "AI 正在思考", compact = false }: { label?: string; compact?: boolean }) {
    ...
    const inkClipId = `moying-ink-${id}`;
    const vermilionClipId = `moying-vermilion-${id}`;

    return (
        <span className={compact ? `${styles.orb} ${styles.compact}` : styles.orb} role="status" aria-label={label}>
